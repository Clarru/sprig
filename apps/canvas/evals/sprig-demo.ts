...
const lines: string[]=JSON.parse(await readFile(new URL('./sprig-demo-narration.json',import.meta.url),'utf8')).beats;
...
try {
 ...
 const bootstrap=await(await page.request.get('http://127.0.0.1:5191/api/bootstrap')).json();
 ...
 const board=async()=>(await(await page.request.get(`http://127.0.0.1:5191/api/agent/boards/${editor}`,{headers})).json()).board as Board;
 if(voice){await page.getByRole('button',{name:'Start listening',exact:true}).click();await page.locator('.cv-listening-control[data-microphone-live="true"]').waitFor();}
 for(const [i,text] of lines.entries()){
  ...
  while((!completed || Date.now()-lastModelEvent<(voice?1500:900) || (voice&&Date.now()-speechEnded<2000)) && Date.now()<deadline){
