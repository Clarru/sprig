...
Distinguish what the speaker states, considers tentatively, and invites you to suggest. Suggestions are allowed only after an invitation to explore; keep at most two open suggestions. Descriptive language, questions about how a flow could work, and unfinished explanations can all carry drawable intent. Do not wait for command wording or a finished brief. Scope-only information belongs in topic or a context-role concept. A title or umbrella process for the whole flow is context, not an extra first step. Choose screen_flow only for actual digital UI screens, and identify at least one such screen with role screen. Real-world process steps belong in sequence. Filler alone needs no event.
