...
export function ListeningControl({status, active, microphone, audioReady, level=0, threshold=0.04, transcript='', moments=[], onToggle, onUndo}: ListeningControlProps) {
  ...
  const live=active && microphone==='live' && audioReady;
  ...
  const phase=status.state==='error' ? 'error'
    : active && !live ? (microphone==='muted'||microphone==='ended'||(microphone==='live'&&!audioReady) ? 'interrupted' : 'connecting')
  ...
  return <div className="cv-listening-control" data-phase={phase} data-microphone-live={live}>
    <AnimatePresence initial={false}>
      {open && <motion.div key="details" ref={panel} id={detailsId} className="cv-listening-details" role="dialog" aria-label="Your conversation with Sprig" tabIndex={-1}
        ...
        <p className="cv-listening-privacy">{live?'Microphone on':'Microphone off'} · No spoken replies</p>
        {transcript && <div className="cv-listening-heard"><span>You’re saying</span><p>{transcript}</p></div>}
        ...
        <ol>{[...moments].reverse().map(moment=><li key={moment.id}>
          {moment.heard && <><span>You said</span><p>{moment.heard}</p></>}
    ...
    <div className="cv-listening-pill">
      <button type="button" className="cv-listening-main" aria-label={active?'Pause listening':'Start listening'} onClick={onToggle}>
        ...
        <span className="cv-listening-copy">
          {reduced ? <strong>{title}</strong> : <AnimatePresence mode="wait" initial={false}>
